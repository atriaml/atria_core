...
def test_key_replaces_dots_and_slashes() -> None:
    instance = DataInstance(sample_id="a/b.c")
...
def test_has_annotation_type_true_when_present() -> None:
    ...
    instance = DataInstance(sample_id="s1").add_annotation(ann)
...
def test_equality() -> None:
    ann = make_classification_annotation()
    a = DataInstance(sample_id="s1").add_annotation(ann)
    b = DataInstance(sample_id="s1").add_annotation(ann)
    c = DataInstance(sample_id="s2").add_annotation(ann)
    assert a == b
    assert a != c
...
def test_annotations_to_dict_from_dict_roundtrip() -> None:
    ...
    restored = DataInstance._annotations_from_dict(data)
...
def test_annotations_from_dict_unknown_type_raises() -> None:
    with pytest.raises(ValueError, match="Unknown annotation type"):
        DataInstance._annotations_from_dict({"not_a_real_type": {}})
