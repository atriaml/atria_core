...
class DocumentInstance(BaseDataInstance):
    ...
    @cached_property
    def document(self) -> Image | PDF:
        if self.file_path.endswith(".pdf"):
            return PDF.from_file_path_or_uri(self.file_path)
        return Image.from_file_path_or_uri(self.file_path)
    ...
    def content(self) -> DocumentContent:
        ...
        if isinstance(document, PDF):
            text_elements = document.extract_text_elements(page_number=self.page_id)
        else:
            ocr_engine = _DEFAULT_OCR_CONFIG.build()
            text_elements = ocr_engine.extract_text_elements(document)
